...
frontend/              — Astro static site (deployed on Netlify)
  src/
    ...
    components/        — Astro components (Header, HeroCircles, LaBoutique, etc.)
...
- **Implementation specs**: Detailed per-section prompts live in `context/*.md`. Before modifying or building a section, always read the corresponding spec first (e.g. `context/front_banner.md` for the header, `context/hero_navigation_cards.md` for the circle cards). These contain exact image processing commands, sizing, colors, font setup, and design decisions.
